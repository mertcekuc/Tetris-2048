...
public class Tetromino {
   ...
   public void rotate() {
      ...
      Tile[][] newTileMatrix = new Tile[tileMatrix.length][tileMatrix.length];
      for (int i = 0; i < tileMatrix.length; i++) {
         for (int j = 0; j < tileMatrix.length; j++) {
            newTileMatrix[j][tileMatrix.length - 1 - i] = tileMatrix[i][j];
         }
      }
      // check if the rotated tetromino goes out of bounds on the left or right
      int newBottomLeftCellX = bottomLeftCell.getX() + (tileMatrix.length - newTileMatrix.length) / 2;
      if (newBottomLeftCellX < 0 || newBottomLeftCellX + newTileMatrix.length > gridWidth) {
         // if it does, don't rotate and return
         return;
      }
      // otherwise, update the tile matrix and the position of the tetromino
